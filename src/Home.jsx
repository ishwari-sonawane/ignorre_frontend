...
const ServiceCard = ({ icon: Icon, title, description, status, buttonText, path }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold text-[#312C85] mb-3">{title}</h3>

      <p className="text-[#585765] text-sm leading-relaxed mb-8 flex-grow">
        {description}
      </p>
      
      {status === 'Coming Soon' ? (
        <div className="text-gray-500 font-medium py-2 px-4 bg-black/5 rounded-lg w-fit text-sm">
          Coming Soon
        </div>
      ) : (
        <button 
          ...
          className="text-[#312C85] text-sm font-medium border border-black/35 py-2 px-4 rounded-lg hover:bg-black/5 transition-all w-fit"
        >
          {buttonText}
        </button>
      )}
  ...
  <div className="text-center max-w-4xl w-full px-4">
    ...
    <div className="mt-8 flex justify-center">
      <div className="w-full max-w-4xl flex flex-col items-center">
        ...
        <div className="w-full max-w-2xl flex flex-col sm:flex-row bg-white rounded-2xl shadow-md overflow-hidden z-10 relative">
          ...
          <button className="bg-[#4F39F6] text-white px-10 py-5 text-sm font-semibold whitespace-nowrap">
            Search
          </button>
        </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-300 shadow-md overflow-hidden">
              ...
              {status === "done" &&
                results.map((item) => (
                  <div
                    key={item.domain}
                    className="flex flex-col sm:flex-row sm:justify-between sm:items-center px-6 py-5 border-b border-gray-200 last:border-b-0 gap-2 min-h-[70px]"
                  >
                    {/* domain name */}
                    <span className="text-gray-700 font-medium break-all text-left">
                      {item.domain}
                    </span>
    ...
    <header className="pt-24 pb-16 px-6 text-center">
      <motion.span
        ...
        className="text-[#312C85] font-semibold text-sm tracking-wider uppercase mb-6 block border border-[#312C85] w-fit mx-auto py-2 rounded-full px-6 "
      >
        Products
      </motion.span>
      <motion.h1
        initial={{ opacity: 0, y: 10 }}
            ...
            <motion.p 
              ...
              className="max-w-2xl mx-auto text-[#585765] text-lg "  
            >
       ...
       <motion.div 
                  ...
                  <div>
                    <div className="flex justify-between items-start mb-8">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-[#312C85] ">{feature.metric}</div>
                        <div className="text-[10px] text-[#949BAD] font-bold tracking-widest uppercase abc">
                          {feature.metricLabel}
                        </div>
                      </div>
                    </div>
                    
                    <h3 className="text-xl text-[#312C85] font-bold mb-4 abc">
                      {feature.title}
                    </h3>
                    <p className="text-[#585765] text-sm leading-relaxed abc">
                      {feature.description}
                    </p>
                  </div>
                </motion.div>
          ...
          Explore More <TiArrowRight className='text-[30px]'/>
               ...
               <div className="min-h-screen bg-[#E5EBFF] text-white py-20 px-4 font-sans selection:bg-blue-500/30 overflow-x-hidden abc">
                      ...
                      <div className="flex items-center gap-4 bg-white p-1.5 rounded-full border border-white/5 relative shadow-sm">
                        <button
                          ...
                          className={`relative z-10 px-6 py-2 text-sm font-medium transition-all duration-300 ${
                            billingCycle === 'monthly' ? 'text-white' : 'text-[#585765]'
                          }`}
                        >
                          Monthly
                        ...
                        <div className="flex items-center gap-2 relative">
                          <button
                            ...
                            className={`relative z-10 px-6 py-2 text-sm font-medium transition-all duration-300 ${
                              billingCycle === 'yearly' ? 'text-white' : 'text-[#585765]'
                            }`}
                          >
                            Yearly
                            {billingCycle === 'yearly' && (
                          ...
                          <span className="bg-blue-600/10 text-blue-500 text-[10px] font-bold px-2 py-0.5 rounded-md border border-blue-500/20">
                            -20%
                          </span>
                        </div>
                      </div>
                        ...
                        >
                          {plan.isPopular && (
                            <motion.div 
                              initial={{ opacity: 0, y: -10 }}
                              animate={{ opacity: 1, y: 0 }}
                              className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#5B4DFF] text-white text-[11px] font-bold tracking-widest uppercase px-5 py-1.5 rounded-full z-10 flex items-center gap-1"
                            >
                          ...
                          <div className="mb-6">
                            <h3 className={`text-3xl font-bold mb-4 ${plan.isPopular ? 'text-gray-100' : 'text-gray-100'}`}>
                              ...
                              <span className="text-[#312C85]">{plan.title}</span>
                            </h3>
                          ...
                          <div className="w-full h-[1px] bg-gray-200/60 my-8" />
              
                          <ul className="space-y-5 mb-10 flex-grow">
                            {plan.features.map((feature, fIdx) => (
                              <li key={fIdx} className="flex items-start gap-4 text-[15px] text-[#5c6475] font-medium">
                                <div className="mt-1">
                                  <FaCheck className="text-[#312C85] text-[14px]" />
                                </div>
                                <span>{feature}</span>
                              </li>
                            ))}
                    ...
                    <a href="/features"> <button className="mt-5 mb-35 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-3 px-8 rounded-full transition-all transform hover:scale-105 flex items-center gap-3">
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="space-y-4">
                {faqs.map((faq, index) => (
                  <div 
                    key={index} 
                    className="border border-[#312C85] rounded-lg overflow-hidden transition-all duration-300 bg-white"
                  >
                    <button
                      onClick={() => toggleAccordion(index)}
                      className="w-full flex items-center justify-between p-5 text-left hover:bg-white/5 transition-colors group"
                    >
                      <span className="text-sm md:text-base pr-4 text-[#312C85] font-bold abc">
                        {faq.question}
                      </span>
                      <FiChevronDown 
                        className={`flex-shrink-0 transition-transform duration-300 text-[#312C85] ${
                    ...
                    >
                      <div className="p-5 pt-0 text-[#585765] text-sm md:text-base leading-relaxed abc">
                        {faq.answer}
                      </div>
                    </div>
                  </div>
              ...
              <div className="mt-12 text-center">
                <p className="text-[#585765] text-[16px] font-semibold abc">
                  Still have questions?{' '}
                  <br />
                  
                  <a href="/contact" className="text-[#312C85] text-[16px] font-semibold abc">
