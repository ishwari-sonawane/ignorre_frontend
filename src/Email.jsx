...
<div className='abc'>
  ...
  >
    ...
    <div className="flex flex-col items-center text-center w-full max-w-[1400px] mx-auto">
      ...
      <p className="mt-8 text-[15px] sm:text-[18px] lg:text-[19px] font-medium text-[#838F8B] leading-relaxed max-w-[720px]">
      ...
      <div className="mt-12">
        <a href="/pricing">
          <button className="cursor-pointer px-10 py-3.5 text-[17px] font-semibold text-white rounded-xl 
            bg-gradient-to-b from-[#5A5A5A] via-[#4A4A4A] to-[#333333] 
...
<div className="w-full bg-[#F9F9F9] py-10 px-4">
  ...
  >
    <img src={logo1} alt="Logo 1" className="h-8 md:h-10 w-auto object-contain" />
      ...
      <div className='bg-[#E3E9FF] abc'>
        <div className="slide-up flex flex-col xl:flex-row items-center justify-center gap-2 2xl:pb-10">
          ...
          <div className="mt-10 px-6 sm:px-10 2xl:ml-0 2xl:mt-30 ">
            <p className="text-[32px] sm:text-[36px] lg:text-[40px] font-semibold text-[#312C85] text-center">
              A complete workspace, built for modern teams
            </p>

            <p className="text-[16px] sm:text-[17px] lg:text-[18px] font-medium text-[#828E8A] text-center 2xl:mt-3">
...
<div className='bg-[#E3E9FF] abc w-full overflow-hidden'>
  ...
  <div className="slide-up py-16 md:py-24 px-4 sm:px-10 max-w-[1400px] mx-auto">
    ...
    <div className="w-full mt-10 lg:mt-20">
      ...
      */}
      <div className="flex flex-col sm:flex-row sm:justify-center border-b border-gray-300 mb-10 overflow-x-auto no-scrollbar"
           style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
        {tabs.map((tab, index) => (
          <button
            key={index}
            onClick={() => setActiveTab(index)}
            className={`pb-4 px-6 text-[16px] lg:text-[18px] font-medium whitespace-nowrap transition-all duration-300 border-b-2 -mb-[2px]
              ${activeTab === index
                ? "text-[#312C85] border-[#312C85]"
      ...
      <div className="relative">
        {tabs.map((tab, index) => (
          ...
          >
            {/* IMAGE */}
            <div className="flex justify-center bg-[#F9F9F9] rounded-3xl overflow-hidden shadow-sm">
              <img
                src={tab.img}
                alt={tab.name}
            ...
            <div className="flex flex-col gap-4 lg:gap-6 text-center lg:text-left items-center lg:items-start">
              <p className="text-[16px] lg:text-[18px] text-[#5178FF] font-semibold uppercase tracking-wide">
                {tab.name}
              </p>

              <h2 className="text-[28px] sm:text-[34px] lg:text-[44px] font-bold text-[#312C85] leading-tight">
                {tab.title}
              </h2>

              <p className="text-[16px] lg:text-[18px] text-gray-600 leading-relaxed max-w-xl">
                {tab.desc}
              </p>
              ...
              <div className="mt-4">
                <a
                  ...
                  className="inline-block py-3 px-10 text-[18px] font-medium text-white rounded-xl bg-gradient-to-b from-[#5A5A5A] via-[#565656] to-[#383838] transition-all duration-200 ease-out hover:brightness-125 hover:shadow-lg"
                >
                  {tab.button.text}
                </a>
              </div>
...
<div className='bg-[#E3E9FF] abc w-full min-h-screen flex flex-col justify-center'>
  <div className="slide-up
    ...
    <div className="w-full text-center md:text-left">
      <p className="text-[32px] sm:text-[40px] lg:text-[48px] font-semibold text-[#312C85] leading-tight max-w-4xl">
    ...
    <div className="mt-10 lg:mt-20 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 2xl:gap-10 justify-items-center">
      ...
      {[
        ...
        <div key={index} className="slide-up min-h-[380px] w-full max-w-[450px] bg-white border border-gray-300 rounded-2xl p-6 flex flex-col justify-between shadow-sm">
          
          <div className="flex flex-col gap-4">
            <FaQuoteLeft size={28} className="text-[#312C85] flex-shrink-0" />
            <p className="text-gray-700 leading-relaxed text-base sm:text-lg italic">
              "{testimonial.text}"
            </p>
          </div>

          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-gray-100 pt-4">
            <div className="flex items-center gap-3">
              <img src={testimonial.img} alt={testimonial.name} className="h-12 w-12 rounded-full object-cover flex-shrink-0" />
              <div className="flex flex-col text-[#312C85] font-semibold text-sm">
                <p className="whitespace-nowrap">{testimonial.name}</p>
                <p className="text-xs opacity-80">{testimonial.co}</p>
              </div>
            </div>
