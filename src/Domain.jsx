...
const handleSearch = (value) => {
  ...
  return (
    ...
    <div className="bg-white text-gray-800">
      ...
      {/* HERO */}
     <section className="w-full min-h-[80vh] flex items-center justify-center bg-[#E3EAFF] abc py-12">
  ...
  <div className="flex items-center bg-white rounded-xl border overflow-hidden focus-within:ring-1 focus-within:ring-indigo-500">
    ...
    <button
      ...
      className="bg-indigo-600 text-white px-6 py-4 font-semibold hover:bg-indigo-700 transition"
    >
      Search
    </button>
  ...
  <div className="w-full max-w-4xl space-y-4">
    {results.map((item, index) => (
     ...
     <p className="mt-6 text-gray-600 text-sm md:text-base">Instant domain search and availability check.<br />
...
>
  {[
    ...
    return (
      <div
        key={i}
        className="group p-6 bg-white border border-gray-100 rounded-2xl hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
      >
        <Icon className="text-3xl text-[#B354FF] mb-4 transition-transform duration-300 group-hover:scale-110" />

        <h4 className="font-semibold text-lg mb-2 text-[#2D2A7A]">
          {feature.title}
        </h4>
 ...
 <div className="py-16 px-6 bg-[#E3EAFF] text-center abc">
      ...
      <div className="max-w-4xl mx-auto space-y-3 mt-8 ">
        {faqs.map((item, i) => (
          <div
            key={i}
            className="border border-[#312C85] rounded-lg overflow-hidden"
          >
            {/* QUESTION */}
            <button
              onClick={() => setActive(active === i ? null : i)}
              className="w-full flex justify-between items-center px-5 py-4 text-left font-bold bg-white text-[16px] text-[#312C85]"
            > 
              {item.q}
  ...
  {active === i ? <FiChevronUp /> : <FiChevronDown />}
            ...
            {/* ANSWER */}
            <div
              className={`px-5 text-[16px] text-[#656774] overflow-hidden transition-all  duration-300 w-full flex justify-between items-center  text-left font-medium bg-white ${
