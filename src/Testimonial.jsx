...
const TestimonialCard = ({ item }) => (
  <div className="inline-block w-[300px] md:w-[400px] mx-3 bg-white border border-white/5 p-6 rounded-xl whitespace-normal ">
    <p className="text-[#949BAD] text-base md:text-lg mb-6 leading-relaxed  abc">
      "{item.content}"
    </p>
    <div className="flex items-center gap-4">
      ...
      />
      <div>
        <h4 className="font-bold text-[#312C85] text-sm md:text-base abc">{item.name}</h4>
        <p className="text-[#312C85] text-xs uppercase tracking-wider abc">{item.role}</p>
      </div>
    </div>
  </div>
);
...
const Testimonial = () => {
  ...
  return (
    <>
    <ScrollToTop />
    <section className="bg-[#E5EBFF] text-white py-24 min-h-screen flex flex-col justify-center relative overflow-hidden">
      {/* Header Section */}
